...
class SettingsController {
  async getSettings(req, res) {
    try {
      ...
      if (data.brapi_token) data.brapi_token_masked = '****' + data.brapi_token.slice(-4);
      if (data.alphavantage_key) data.alphavantage_key_masked = '****' + data.alphavantage_key.slice(-4);
      if (data.groq_api_key) data.groq_api_key_masked = '****' + data.groq_api_key.slice(-4);
  ...
  async updateSettings(req, res) {
    try {
      const { rebalanceThreshold, investmentHorizon, riskProfile, monthlyContribution, brapiToken, alphavantageKey, groqApiKey } = req.body;
      ...
      `, [rebalanceThreshold, investmentHorizon, riskProfile, monthlyContribution, brapiToken, alphavantageKey, groqApiKey, req.userId]);
      ...
      if (result.rows.length === 0) {
        ...
        `, [req.userId, rebalanceThreshold || 5, investmentHorizon || 10, riskProfile || 'moderate', monthlyContribution || 0, brapiToken, alphavantageKey, groqApiKey]);
      }
  ...
  async testApiConnection(req, res) {
    try {
      ...
      if (api === 'brapi') {
        try {
          const response = await axios.get(`https://brapi.dev/api/quote/PETR4?token=${token}`, { timeout: 10000 });
          if (response.data?.results?.[0]?.regularMarketPrice) {
            result = { success: true, message: `Conexão OK. PETR4: R$ ${response.data.results[0].regularMarketPrice}` };
          } else {
            result = { success: false, message: 'Token inválido ou sem resposta' };
          }
        } catch (e) {
          result = { success: false, message: 'Erro ao conectar com Brapi: ' + (e.response?.data?.message || e.message) };
        }
      } else if (api === 'alphavantage') {
        try {
          const response = await axios.get(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=AAPL&apikey=${token}`, { timeout: 10000 });
          if (response.data?.['Global Quote']?.['05. price']) {
            result = { success: true, message: `Conexão OK. AAPL: $ ${response.data['Global Quote']['05. price']}` };
          } else {
            result = { success: false, message: 'API key inválida ou limite excedido' };
          }
        } catch (e) {
          result = { success: false, message: 'Erro ao conectar com Alpha Vantage' };
        }
      } else if (api === 'groq') {
        try {
